...
--
-- DNI format: naver-stock-tracker:<slot>
...
local DEVICE_PREFIX = "naver-stock-tracker:"
...
local function find_max_slot(driver)
  ...
  for _, d in ipairs(driver:get_devices()) do
    ...
    local n = tonumber(dni:match("^naver%-stock%-tracker:(%d+)$"))
...
local function create_device(driver, slot)
  ...
  driver:try_create_device({
    ...
    label             = "Naver Stock Tracker",
    profile           = "main",
    manufacturer      = "SmartThings",
    model             = "Stock Tracker",
    vendor_provided_label = "Naver Stock Tracker",
  })
end
