...
-- Only encodes characters that MUST be encoded in a query-string value
-- but leaves existing %-sequences intact (i.e. does NOT encode '%' itself).
-- Use this when the target_url was built from an already-encoded API key.
...
function M.forward(ip, port, target_url, headers)
  local path = "/api/forward?url=" .. urlencode_url(target_url)
